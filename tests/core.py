...
class TensorNode(
    ...
    def __init__(self, value: np.ndarray, source=None):
        ...
        self.grad = np.zeros(
            shape=value.shape
        )  # This is where we will put the gradient when we compute the loss
...
class OpNode:
    ...
    def backward(self):
        ...
        for node, grad in zip(self.inputs, ginputs_raw):
            assert (
                node.grad.shape == grad.shape
            ), f"node shape is {node.size()} but grad shape is {grad.shape}"
...
class Op:
    ...
    def do_forward(cls, *inputs, **kwargs):
        ...
        assert all(
            [type(input) == TensorNode for input in inputs]
        ), "All inputs to an Op should be TensorNodes."
